...
文件位置：`extensions/scripts/ai_chat/workflows/*.yaml`（每个工作流一个文件，便于独立维护）。
...
4. 配置文件放 `extensions/scripts/ai_chat/workflows/`。✅ 已实现。

## 11. 实现记录（一期落地）

- 后端框架：`src/extensions_host/workflow_engine.py`（加载 yaml / 关键词推断 / 编译 prompt / 执行 shell 步）。
- 配置：`extensions/scripts/ai_chat/workflows/{defect,suggest,resume,chat}.yaml`。
