...
def _load_codebuddy_token() -> str:
    """从通用凭证保险库读取 codebuddy token（集中管理）。
    ...
    try:
        ...
        vault_tok = vault.get_token(domain=CODEBUDDY_TOKEN_DOMAIN)
        if not vault_tok:
            vault_tok = vault.get_apikey(domain=CODEBUDDY_TOKEN_DOMAIN)
        if vault_tok:
